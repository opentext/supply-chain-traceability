...
export default class Publications {
  ...
  publicationUrl =
    process.env.REACT_APP_BASE_URL + "/publication/api/v1/publications";

  constructor(props) {
    this.props = props;
  }

  getPublicationRequestBody(id, mime_type, name) {
    return {
  ...
  async waitForPublicationComplete(id, fileId, fileCategory, callback) {
  ...
  async getPublicationData(fileId, fileCategory, callback) {
    ...
    publicationData = await fetch(
      ...
      .then(async (result) => {
        ...
        if (blobId) {
          let body = this.getPublicationRequestBody(blobId, mime_type, name);
