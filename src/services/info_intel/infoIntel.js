...
export async function getOCRdata(file, setExtractionData, accessToken) {
  const formData = new FormData();
  formData.append('File', file);
  let resultsData = {};
  const config = {
    method: 'post',
    url: `${reactBaseUrl}/mtm-riskguard/api/v1/extract?action=ocr`,
    headers: {
...
function extractResults(text, name) {
  let resultData = {};
...
function extractSaleInvoiceData(text) {
  ...
  resultData.push({
    ...
    value: sellerCompanyMatch ? sellerCompanyMatch[1].trim() : null,
    type: 'input',
  });
  resultData.push({
    label: 'Invoice Date',
...
function extractShipmentData(documentText) {
  ...
  const reportData = {
    carrierName: extractData(
      /Carrier Name:\s*([^ ]+.*?)(?=\s*Date of Issue:)/,
      documentText,
    ),
    shipperName: extractData(
      /Shipper Name:\s*([^ ]+.*?)(?=\s*Consignee Name:)/,
      documentText,
    ),
    consigneeName: extractData(/Consignee Name:\s*([^\n]+)/, documentText),
    estimatedArrivalDate: extractData(
      /Estimated Arrival Date:\s*([^ ]+.*?)(?=\s*Cost)/,
      documentText,
    ),
  };
  resultData.push({
...
function extractWarehouseData(documentText) {
  ...
  const reportData = {
    warehouseName: extractData(
      /Warehouse Name:\s*([^ ]+.*?)(?=\s*Receipt Number:)/,
      documentText,
    ),
    warehouseLocation: extractData(
      /Address:\s*([^ ]+.*?)(?=\s*Date of Receipt:)/,
      documentText,
    ),
    totalCost: extractData(
      /Total Cost \(Including Insurance\):\s*([\d,]+)\s*USD/,
      documentText,
    ),
  };
  resultData.push({
...
function extractReportDataGerman(documentText) {
  const resultData = [];
  const reportData = {
    analystName: extractData(
      /Name des Analysten\s*:\s*([^\n]+?)(?=\s+Berichts-ID)/,
      documentText,
    ),
    assessmentDate: extractData(
      /Datum\s*:\s*([A-Za-z0-9,\s]+)(?=\n)/,
      documentText,
    ),
    reportId: extractData(/Berichts-ID\s*:\s*([A-Za-z0-9]+)/, documentText),
    numberOfAnalyticalMethods: extractData(
      /wir (.+) Analysemethoden/,
      documentText,
    ),
